import { Component, ElementRef, EventEmitter, HostListener, Output, ViewChild, NgZone, inject } from '@angular/core';
...
@Component({
  ...
  templateUrl: './scan-panel.component.html',
  styleUrls: ['./scan-panel.component.scss']
})
export class ScanPanelComponent {
  ...
  private caricaBox(id: string) {
    ...
    this.dbService.getBoxSingola(numId).subscribe({
      next: (box: any) => {
        if (box) {
          const alreadyExists = this.boxTrovati.some(b => b.id === box.id);
          if (!alreadyExists) {
            this.boxTrovati = [...this.boxTrovati, box];
          }
          this.caricamentoError = '';
        } else {
      ...
      img.onload = () => {
        ...
        try {
          ...
          if (code && code.data) {
            ...
            this.ngZone.run(() => alert('Nessun QR Code trovato nell\'immagine.'));
          }
        } catch {
          this.ngZone.run(() => alert('Errore nella lettura dell\'immagine.'));
        }
      };
      img.onerror = () => {
        this.ngZone.run(() => alert('Impossibile leggere l\'immagine selezionata.'));
      };
      img.src = `data:${mime};base64,${b64}`;
    } catch {
      this.ngZone.run(() => alert('Errore nell\'aprire la galleria.'));
    }
  }
